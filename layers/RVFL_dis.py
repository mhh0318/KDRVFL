...
class RVFL_layer(nn.Module):
    ...
    def matrix_inverse(self, x, target, target_teacher):
    ...
    def matrix_inverse_raw(self, x, target):
        n_sample, n_D = x.shape
        if n_D<n_sample:
    ...
    def train(self, X, target, target_t, return_loss=False):
        raw_X = X
        n_sample, n_D = X.shape
        # print(X.shape)
        ...
        self.A_ = A_
        beta_ = self.matrix_inverse(A_merge, target, target_t)
        self.beta = beta_
        ...
        if return_loss is True:
            L2_loss = F.mse_loss(predict_score,target)
            return L2_loss
